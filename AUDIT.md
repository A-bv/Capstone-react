...
10. **Polish**: modal focus trap + focus restore + close-on-route-change; rating `aria-label`; convert `#about`/`#contact` anchors to buttons + add catch-all route; dynamic footer year. [L3] ✅ _done across the nav, modal, and this final polish commit (focus trap/restore, rating labels, buttons + catch-all, current-year footer). Close-on-route-change intentionally skipped (rationale above). Also removed the now-stale `.js` path-header comments left by the #11 rename._
